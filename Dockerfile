...
WORKDIR /home/srophe-docker
COPY . .

# Build Srophe application code
WORKDIR /home/srophe-docker/srophe
RUN ant

# Build Srophe data application
WORKDIR /home/srophe-docker/srophe-app-data
RUN ant
...
# Copy custom conf.xml to exist. This adds the RDF indexer for the SPARQL module. 
COPY conf/conf.xml /exist/

COPY --from=builder /home/srophe-docker/srophe/build/*.xar /exist/autodeploy
COPY --from=builder /home/srophe-docker/srophe-app-data/build/*.xar /exist/autodeploy
